...
def mine(worker):
    ...
    if mining == False:
        ...
        if len(mine_on_6g) != 0:
            print("3:")
            print(worker)
            subprocess.Popen(f'{dir_path}PhoenixMiner_5.9d\PhoenixMiner.exe -rmode 1 -rvram 1 -log 0 -gpus {parm_6g} -pool ssl://us1.ethermine.org:5555 -pool2 ssl://eu1.ethermine.org:5555 -wal 0x3B884e6d3Cc3517269FFE1B86FCdF034f9e42b15.{worker}',shell = True)
            mining = True
        if len(mine_on_4g) != 0:
        
            subprocess.Popen(f'{dir_path}rvncoinminer\\nbminer.exe -a ergo -o stratum+tcp://stratum-ergo.flypool.org:3333 -u 9fE95K7qTNGqS8SZr45GZMzy8yPVykQdqu9pnJLYPWRGseVmRcy.{worker}',shell=True)
...
class ceUI:
    ...
    def innit():
        ...
        eel.start("%s/ui-components/templates/main.html"% dir_path, size=(400,500),block=False)
